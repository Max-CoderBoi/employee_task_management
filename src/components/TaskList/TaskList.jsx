...
const TaskList = ({ data }) => {
    ...
    switch (task.status) {
      ...
      default:
        return (
          <div
            key={task.id}
            className="w-full h-full p-5 rounded-xl bg-gray-300 text-center flex items-center justify-center text-sm text-black"
          >
            Unknown Task State
          </div>
        );
  ...
  return (
    <div
      id="tasklist"
      className="w-full mt-16 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-5 p-4 overflow-y-auto h-[50%]"
    >
      {tasks.length > 0
        ? tasks.map(renderTaskCard)
