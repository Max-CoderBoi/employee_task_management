...
const AllTask = () => {
  ...
  return (
    <div className="bg-[#1a1a1a] p-6 rounded-2xl mt-6 shadow-xl border border-white/10">
      <h2 className="text-2xl font-bold text-white mb-6 border-b border-gray-700 pb-3">
        📊 Task Summary Overview
      </h2>

      {/* Table Header */}
      <div className="grid grid-cols-2 sm:grid-cols-5 text-white font-semibold text-xs sm:text-sm bg-gradient-to-r from-emerald-600 to-emerald-500 py-3 px-4 rounded-t-lg shadow-sm">
      ...
      {/* Table Body */}
      <div className="divide-y divide-gray-700 bg-[#121212] rounded-b-lg overflow-hidden">
        {userData.map((user, idx) => (
          <div
            key={idx}
            className="grid grid-cols-2 sm:grid-cols-5 text-xs sm:text-sm text-white text-center py-3 px-4 hover:bg-white/10 transition duration-200 ease-in-out"
          >
            <div className="font-medium">{user.firstName}</div>
            <div className="text-blue-400 font-semibold">{user.taskCounts.newTask}</div>
            <div className="text-yellow-300 hidden sm:block font-semibold">{user.taskCounts.active}</div>
            <div className="text-green-400 hidden sm:block font-semibold">{user.taskCounts.completed}</div>
            <div className="text-red-500 hidden sm:block font-semibold">{user.taskCounts.failed}</div>
          </div>
        ))}
      </div>
    </div>
